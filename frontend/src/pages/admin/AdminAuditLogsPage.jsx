    ...
    <div className="space-y-6 print:p-0" data-testid="admin-audit-logs">
      <header className="flex flex-wrap items-start justify-between gap-4 print:hidden">
        <div>
          ...
          <h1 className="font-heading text-3xl sm:text-4xl font-semibold text-[#3E2723] mt-2 flex items-center gap-3">
            <History size={28} /> {t("audit.title")}
          </h1>
          <p className="mt-2 text-[#795C55]">{t("audit.subtitle")}</p>
        </div>
        <div className="flex gap-2">
          ...
          >
            <Download size={16}/> {t("audit.export_csv")}
          </button>
          <button
      ...
      {summary && (
        <section className="grid sm:grid-cols-3 gap-4">
          <StatCard label={t("audit.total")} value={summary.total} />
          <StatCard label={t("audit.distinct_actions")} value={summary.by_action.length} />
          <StatCard label={t("audit.distinct_entities")} value={summary.by_entity.length} />
        </section>
      )}

      <section className="bg-white rounded-2xl border border-[#3E2723]/5 p-4 grid sm:grid-cols-3 gap-3 print:hidden" data-testid="audit-filters">
        <div>
          <label className="text-[11px] uppercase tracking-wider text-[#795C55] flex items-center gap-1 mb-1"><Filter size={12}/> {t("audit.filter_action")}</label>
          <Select value={filters.action || "all"} onValueChange={(v) => setFilters(f => ({ ...f, action: v === "all" ? "" : v }))}>
            <SelectTrigger className="h-10 rounded-lg"><SelectValue placeholder={t("audit.filter_all")} /></SelectTrigger>
            <SelectContent>
        ...
        <div>
          <label className="text-[11px] uppercase tracking-wider text-[#795C55] flex items-center gap-1 mb-1"><ListChecks size={12}/> {t("audit.filter_entity")}</label>
        ...
        <div>
          <label className="text-[11px] uppercase tracking-wider text-[#795C55] flex items-center gap-1 mb-1"><User2 size={12}/> {t("audit.filter_user_id")}</label>
        ...
        <table className="w-full text-sm">
          <thead className="bg-[#F7F3EB]">
            <tr className="text-left text-xs uppercase tracking-wider text-[#795C55]">
              <th className="px-4 py-3">{t("audit.col_date")}</th>
              <th className="px-4 py-3">{t("audit.col_user")}</th>
              <th className="px-4 py-3">{t("audit.col_action")}</th>
              <th className="px-4 py-3">{t("audit.col_entity")}</th>
              <th className="px-4 py-3">{t("audit.col_id")}</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-[#3E2723]/5" data-testid="audit-rows">
            {loading ? (
              <tr><td colSpan={5} className="px-4 py-6 text-[#795C55]">{t("loading")}</td></tr>
            ) : logs.length === 0 ? (
              <tr><td colSpan={5} className="px-4 py-6 text-[#795C55]">{t("audit.empty")}</td></tr>
            ) : logs.map(l => (
              <tr key={l.id} data-testid={`audit-row-${l.id}`}>
                <td className="px-4 py-2 text-[#795C55] whitespace-nowrap">{new Date(l.created_at).toLocaleString()}</td>
